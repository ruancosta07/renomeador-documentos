...
const Folder = ({
  ...
  return (
    <li className="text-zinc-100 flex gap-[1rem] w-full relative">
      {status === "converting" ? (
        <FolderUp className="size-[4rem] stroke-1" />
      ) : (
        <FolderCheck className="size-[4rem] stroke-1" />
      )}
      <div className="w-full grid grid-cols-[1fr_auto] gap-[1rem]">
        <div className="">
          <div className="flex items-center gap-[1rem]">
            ...
            <p className="text-zinc-400 text-[1.15rem]">
              {dayjs(uploadedAt).fromNow()}
            </p>
          </div>
          {status === "converting" && (
            <div className="flex items-center gap-[1rem] text-[1.4rem]">
              <div className="bg-zinc-700 w-full h-2 mt-[.6rem] rounded-full relative overflow-x-hidden">
                <div
          ...
          {status === "finished" && (
            <button
              onClick={openFolder}
              className="text-zinc-300 py-[.8rem] text-[1.2rem] cursor-pointer"
            >
              Abrir pasta
            </button>
          )}
        ...
        >
          <Trash2Icon className="size-[2rem]" />
        </button>
